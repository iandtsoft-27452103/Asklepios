...
def main():
    ...
    try:
        ...
        model_value = value_lazy.value_lazy()
        ...
        temp1 = torch.zeros((1,105,9,9), dtype=torch.float, device=device)
        model.eval()
        ...
        with torch.no_grad():
            _ = model.forward(temp1)
            model_value.batch_size = 1
            _ = model_value.forward(temp1)
        ...
        while True:
            ...
            for i in range(limit):
                ...
                if str_mode == 'p':
                    ...
                    if len(moves) == 0:
                        ...
                        fe = ft.make_input_features4(cls_sfen.board, cls_sfen.board.color)
                        li1.append(fe)
                        cnt += 1
                elif str_mode == 'v':
                    fe = ft.make_input_features4(cls_sfen.board, cls_sfen.board.color)
            ...
            if str_mode == 'p':
                length = len(li1)
                li1 = numpy.array(li1)
                x = torch.tensor(li1, dtype = torch.float)
                x = x.reshape(length, 105, 9, 9)
                x = x.to(device)               
                model.eval()
                with torch.no_grad():
                    if length > 0:
                        y = model.forward(x)
                        y = y.reshape(cnt, 32, 81)
                        y = y.tolist()
